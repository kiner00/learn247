<template>
    <AppLayout title="My Affiliates">
        <div class="max-w-5xl">
            ...
            <!-- Header + tabs -->
            <div class="mb-6">
                <h1 class="text-2xl font-bold text-gray-900 mb-4">My Affiliates</h1>
            ...
            <div v-if="activeTab === 'links'">
                <p class="text-sm text-gray-500 mb-5">
                    Share your referral links. When someone subscribes through your link, you earn the community's commission.
                </p>

                <div v-if="affiliates.length === 0" class="text-center py-16 bg-white rounded-2xl border border-gray-200">
                ...
                <div v-if="affiliates.length > 0" class="bg-white rounded-2xl border border-gray-200 overflow-hidden">
                    <table class="w-full text-sm">
                        ...
                        <tbody class="divide-y divide-gray-100">
                            <tr v-for="a in affiliates" :key="a.id" :class="['hover:bg-gray-50', !a.is_active ? 'opacity-60' : '']">
                                <td class="px-5 py-4 font-medium text-gray-900">
                                    <div class="flex items-center gap-2">
                                        <Link :href="`/communities/${a.community.slug}`" class="hover:text-indigo-600">
                                            {{ a.community.name }}
                                        </Link>
                                        <span v-if="!a.is_active" class="text-xs font-bold uppercase bg-red-100 text-red-600 px-2 py-0.5 rounded-full">
                                            Suspended
                                        </span>
                                    </div>
                                </td>
                                <td class="px-5 py-4">
                                    <div v-if="a.is_active" class="flex items-center gap-2">
                                        <span class="font-mono text-xs text-gray-500 truncate max-w-48">{{ a.referral_url }}</span>
                                        <button @click="copy(a.referral_url)" class="shrink-0 text-xs text-indigo-600 hover:text-indigo-800 font-medium">
                                            {{ copied === a.referral_url ? '✓ Copied' : 'Copy' }}
                                        </button>
                                    </div>
                                    <span v-else class="text-xs text-red-500 italic">Renew subscription to reactivate</span>
                                </td>
                                <td class="px-5 py-4 text-right text-gray-900 font-medium">₱{{ fmt(a.total_earned) }}</td>
                                <td class="px-5 py-4 text-right text-gray-500">₱{{ fmt(a.total_paid) }}</td>
            ...
            <div v-if="activeTab === 'analytics'">
                ...
                <div class="flex items-center gap-2 mb-5">
                    ...
                    >
                        ...
                        <option v-for="c in analytics.communities" :key="c.id" :value="c.id">{{ c.name }}</option>
                    </select>
                    ...
                    <div class="flex rounded-lg border border-gray-200 overflow-hidden">
                        <button
                            ...
                            :class="period === p.value ? 'bg-indigo-600 text-white' : 'bg-white text-gray-600 hover:bg-gray-50'"
                        >
                            {{ p.label }}
                        </button>
                    </div>
                ...
                <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-3 mb-5">
                    <div class="bg-white border border-gray-200 rounded-2xl p-4 text-center">
                        ...
                        <p class="text-lg font-bold text-gray-900">₱{{ fmt(analytics.summary.total_earned) }}</p>
                    </div>
                    <div class="bg-white border border-gray-200 rounded-2xl p-4 text-center">
                        <p class="text-xs text-gray-400 mb-1">Paid Out</p>
                        <p class="text-lg font-bold text-green-600">₱{{ fmt(analytics.summary.total_paid) }}</p>
                    </div>
                    <div class="bg-white border border-gray-200 rounded-2xl p-4 text-center">
                        <p class="text-xs text-gray-400 mb-1">Pending</p>
                        <p class="text-lg font-bold text-amber-500">₱{{ fmt(analytics.summary.total_pending) }}</p>
                    </div>
                    <div class="bg-white border border-gray-200 rounded-2xl p-4 text-center">
                        <p class="text-xs text-gray-400 mb-1">Conversions</p>
                        <p class="text-lg font-bold text-gray-900">{{ analytics.summary.total_conversions }}</p>
                    </div>
                    <div class="bg-white border border-gray-200 rounded-2xl p-4 text-center">
                        <p class="text-xs text-gray-400 mb-1">Avg / Referral</p>
                        <p class="text-lg font-bold text-indigo-600">₱{{ fmt(analytics.summary.avg_per_referral) }}</p>
                    </div>
                    <div class="bg-white border border-gray-200 rounded-2xl p-4 text-center">
                        <p class="text-xs text-gray-400 mb-1">Best Month</p>
                        <p class="text-sm font-bold text-gray-900">{{ analytics.summary.best_month ?? '—' }}</p>
                        <p v-if="analytics.summary.best_month_total" class="text-xs text-gray-500">₱{{ fmt(analytics.summary.best_month_total) }}</p>
                    </div>
                </div>
                ...
                <div class="grid grid-cols-1 lg:grid-cols-3 gap-5 mb-5">
                    ...
                    <div class="bg-white border border-gray-200 rounded-2xl p-5">
                        ...
                        <div v-if="analytics.byComm.length" class="space-y-3">
                            <div v-for="row in analytics.byComm" :key="row.community">
                                ...
                                <div class="h-1.5 bg-gray-100 rounded-full overflow-hidden">
                                    <div class="h-full bg-indigo-400 rounded-full" :style="{ width: `${(row.total / byCommMax) * 100}%` }" />
                                </div>
                            </div>
                        </div>
                ...
                <div class="bg-white border border-gray-200 rounded-2xl p-5">
                    <div class="flex items-center justify-between mb-4">
                        <h2 class="text-sm font-semibold text-gray-900">
                            Conversion History <span class="text-gray-400 font-normal">(last 100)</span>
                        </h2>
                        ...
                        <table class="w-full text-sm">
                            ...
                            <tbody class="divide-y divide-gray-50">
                                <tr v-for="c in filteredConversions" :key="c.id" class="hover:bg-gray-50">
                                    ...
                                    <td class="py-2.5 text-gray-800 font-medium max-w-40 truncate">{{ c.community }}</td>
                    ...
                    <p v-else class="text-sm text-gray-400 py-6 text-center">No conversions yet.</p>
                </div>
            </div>
        </div>
                ...
                <div class="bg-white rounded-2xl shadow-2xl w-full max-w-sm p-6">
                    ...
                    <form @submit.prevent="savePayout" class="space-y-3">
                        <div>
                            ...
                            <select v-model="payoutForm.payout_method" class="w-full border border-gray-200 rounded-xl px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500">
                        ...
                        <div>
                            ...
                            <input v-model="payoutForm.payout_details" type="text" placeholder="e.g. 09xxxxxxxxx"
                                class="w-full border border-gray-200 rounded-xl px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500" />
                        </div>
                        <div class="flex gap-3 pt-1">
                            <button type="button" @click="showPayoutModal = false" class="flex-1 py-2 border border-gray-200 text-gray-600 text-sm rounded-xl hover:bg-gray-50">Cancel</button>
                            <button type="submit" class="flex-1 py-2 bg-indigo-600 text-white text-sm font-semibold rounded-xl hover:bg-indigo-700">Save</button>
                        </div>
                    </form>
                </div>
            </div>
...
const filteredConversions = computed(() =>
    statusFilter.value === 'all'
        ...
        : props.analytics.conversions.filter(c => c.status === statusFilter.value)
);

const byCommMax = computed(() =>
    props.analytics.byComm.length ? Math.max(...props.analytics.byComm.map(r => r.total)) : 1
);
